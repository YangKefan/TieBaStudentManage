    ...
    ui(new Ui::RegisterDialog)
{
    ...
    //设置注册对话框的背景
  /*  QPalette p;
    p.setBrush(QPalette::Background,QBrush(QPixmap(":/StudentSystem/img/loginbk1.jpg").scaled(this->size())));
    this->setPalette(p);*/
    //设置注册对话框的标题颜色
    QPalette p1;
    ...
    //设置注册对话框标题
    this->setWindowTitle(GBK::a2w("注册用户"));
    //设置两个按钮的图案
    ui->registerbtn->setIcon(QIcon(":/StudentSystem/img/Enter.png"));
    ui->returnbtn->setIcon(QIcon(":/StudentSystem/img/exit.png"));
    //设置两行密码lineedit的显示
    ui->passwordline->setEchoMode(QLineEdit::PasswordEchoOnEdit);
    ui->passwordline1->setEchoMode(QLineEdit::PasswordEchoOnEdit);
    //设置lineedit提示语句
    ui->userline->setPlaceholderText(GBK::a2w("请输入用户名"));
    ui->studentnumline->setPlaceholderText(GBK::a2w("请输入工作号"));
    ui->passwordline->setPlaceholderText(GBK::a2w("请输入密码"));
    ui->passwordline1->setPlaceholderText(GBK::a2w("请重新输入密码"));
    ui->yanzhengline->setPlaceholderText(GBK::a2w("请输入验证码"));
    //设置注册按钮不可用
...
void RegisterDialog::registerbtnSlot()
{
    ...
    {
        QMessageBox::warning(this,GBK::a2w("警告"),GBK::a2w("验证码输入有误"),QMessageBox::Yes);
        ui->passwordline->clear();
        ui->passwordline1->clear();
        ui->yanzhengline->clear();
        this->setyanzheng();
    ...
    if(radiogroup->checkedId()==0)
    {
        ...
        QMessageBox::information(this,GBK::a2w("提示"),GBK::a2w("注册失败，请重新注册"),QMessageBox::Yes);
        this->clearAll();
        this->setyanzheng();
    }
    //判断是否老师注册
    else if(radiogroup->checkedId()==1)
    {
        ...
        QMessageBox::information(this,GBK::a2w("警告"),GBK::a2w("用户已存在"),QMessageBox::Yes);
        this->clearAll();
        this->setyanzheng();
    }
    else
...
bool RegisterDialog::judgeEmpty()
{
    ...
    {
        QMessageBox::warning(this,GBK::a2w("警告"),GBK::a2w("用户编号不能为空"));
        return false;
    }
    ...
    {
        QMessageBox::warning(this,GBK::a2w("警告"),GBK::a2w("验证码不能为空"));
        return false;
    }
    else
        return true;
