    ...
    ui(new Ui::TeacherManage)
{
    ...
    //设置教师成绩管理界面背景
  /*  QPalette p;
    p.setBrush(QPalette::Background,QBrush(QPixmap(":/StudentSystem/img/studentbk19.jpg").scaled(this->size())));
    this->setPalette(p);*/
    ...
    //设置教师成绩管理界面的标题
    this->setWindowTitle(GBK::a2w("教师成绩管理"));

    //设置教师成绩管理界面的图标
    this->setWindowIcon(QIcon(":/StudentSystem/img/book.jpg"));
    //设置定时器
    ...
    //设置按钮图标
    ui->searchbtn->setIcon(QIcon(":/StudentSystem/img/m5.png"));
    ui->searchbtn->setFlat(true);
    ui->changebtn->setIcon(QIcon(":/StudentSystem/img/m1.png"));
    ui->changebtn->setFlat(true);
    ui->insertbtn->setIcon(QIcon(":/StudentSystem/img/m13.png"));
    ui->insertbtn->setFlat(true);
    ui->deletebtn->setIcon(QIcon(":/StudentSystem/img/m3.png"));
    ui->deletebtn->setFlat(true);
    ui->passwordbtn->setIcon(QIcon(":/StudentSystem/img/m4.png"));
    ui->passwordbtn->setFlat(true);
    ui->returnbtn->setIcon(QIcon(":/StudentSystem/img/m6.png"));
    ui->returnbtn->setFlat(true);
    ui->exitbtn->setIcon(QIcon(":/StudentSystem/img/m7.png"));
    ui->exitbtn->setFlat(true);

    //设置toolbox标题
    ui->teachertool->setItemText(0,GBK::a2w("成绩管理"));
    ui->teachertool->setItemText(1,GBK::a2w("个人中心"));
...
{
    if(QMessageBox::question(this,GBK::a2w("提示"),GBK::a2w("是否注销用户?"),
		QMessageBox::Yes|QMessageBox::No)==QMessageBox::Yes)
    {
    emit toLoginDialog();
    delete this;
