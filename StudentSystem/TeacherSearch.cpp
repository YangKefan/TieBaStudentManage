    ...
    ui(new Ui::TeacherSearch)
{
    ...
    //设置按钮图标
    //ui->searchbtn->setIcon(QIcon(":/StudentSystem/img/m1.png"));
    //ui->searchbtn->setFlat(true);
    //ui->returnbtn->setIcon(QIcon(":/StudentSystem/img/m8.png"));
    //ui->returnbtn->setFlat(true);

    //设置inputedit提示显示
    ui->inputline->setPlaceholderText(GBK::a2w("请输入学生学号或者学生姓名"));
  ...
  /*  model->removeColumns(5,5);*/
    ...
    _model->setHeaderData(0,Qt::Horizontal,GBK::a2w("学号"));
    _model->setHeaderData(1,Qt::Horizontal,GBK::a2w("学生姓名"));
    _model->setHeaderData(2,Qt::Horizontal,GBK::a2w("语文成绩"));
    _model->setHeaderData(3,Qt::Horizontal,GBK::a2w("数学成绩"));
    _model->setHeaderData(4,Qt::Horizontal,GBK::a2w("英语成绩"));
...
{
    model->select();
	qDebug()<<model->data(model->index(0,0,QModelIndex()));
