    ...
    ui(new Ui::LoginDialog)
{
    ...
    //设置登录对话框的标题
    this->setWindowTitle(GBK::a2w("登录"));
    //设置登录对话框的图标
    ...
    //QPalette palette;
    //palette.setBrush(QPalette::Background,QBrush(QPixmap(":/StudentSystem/img/loginbk1.jpg").scaled(this->size())));
    //this->setPalette(palette);
    ////设置三个按钮的图标
    ui->exitbtn->setIcon(QIcon(":/StudentSystem/img/exit.png"));
    ui->registerbtn->setIcon(QIcon(":/StudentSystem/img/log.png"));
    ui->loginbtn->setIcon(QIcon(":/StudentSystem/img/Enter.png"));
    //设置groupbox，放置两个radiobutton
    radiogroup=new QButtonGroup(this);
    radiogroup->addButton(ui->studentradio,0);
    radiogroup->addButton(ui->teacherradio,1);
    //设置图片
    ui->piclabel->setPixmap(QPixmap(":/StudentSystem/img/log.png").scaled(ui->piclabel->size()));
    //设置登录按钮不可用
